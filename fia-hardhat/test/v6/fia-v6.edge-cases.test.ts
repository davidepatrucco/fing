...
describe('FIACoinV6 - Edge Cases and Error Conditions', function () {
  ...
  describe('Governance Edge Cases', function () {
    ...
    it('should fail executing proposal before voting period ends', async function () {
      ...
      await expect(fia.execute(0)).to.be.revertedWith('Voting period not ended');
    });
    ...
    it('should fail executing proposal without quorum', async function () {
      ...
      await expect(fia.execute(0)).to.be.revertedWith('Quorum not met');
    });
    ...
    it('should fail proposing without enough tokens', async function () {
      ...
      await expect((fia.connect(charlie)).propose("Test proposal"))
  ...
  describe('Transfer Edge Cases', function () {
    ...
    it('should fail batchTransfer with empty arrays', async function () {
      ...
      await expect((fia.connect(alice)).batchTransfer([], []))
  ...
  describe('Fee and Limit Edge Cases', function () {
    ...
    it('should handle fee distribution with all percentages set to zero', async function () {
      ...
      // Set all fee distributions to 0
  ...
  describe('Admin Function Edge Cases', function () {
    it('should fail admin functions when called by non-admin', async function () {
      ...
      await expect((fia.connect(alice)).setFeeExempt(bob.address, true))
        .to.be.revertedWith('Admin access required');
      
      await expect((fia.connect(alice)).setFeeDistribution(100, 100, 100))
        .to.be.revertedWith('Admin access required');
      
      await expect((fia.connect(alice)).addToRewardPool(ethers.parseUnits('1000', 18)))
        .to.be.revertedWith('Admin access required');
    });
    ...
    it('should handle setFeeDistribution with maximum values', async function () {
      ...
      // Set maximum allowed values (100% each would exceed 100%, but test individually)
      await fia.setFeeDistribution(10000, 0, 0); // 100% to treasury
      
      let config = await fia.feeConfig();
      expect(config.treasuryShare).to.equal(10000);
      
      await fia.setFeeDistribution(0, 10000, 0); // 100% to founder
      config = await fia.feeConfig();
      expect(config.founderShare).to.equal(10000);
      
      await fia.setFeeDistribution(0, 0, 10000); // 100% to burn
      config = await fia.feeConfig();
      expect(config.burnShare).to.equal(10000);
    });
