...
describe('FIACoinV6 - Advanced Transfer Features', function () {
  ...
  describe('ProtectedTransfer Function', function () {
    it('should prevent replay attacks with nonce tracking', async function () {
      ...
      // First transfer should succeed (Fingered event with net amount after fees)
      const tx = await (fia.connect(alice)).protectedTransfer(bob.address, amount, nonce);
      const receipt = await tx.wait();
      // Ensure a Fingered event was emitted for sendAmount
      const fingered = receipt?.logs.map((l: any) => {
        try { return fia.interface.parseLog(l); } catch { return null; }
      }).filter(Boolean).find((p: any) => p.name === 'Fingered');
      expect(fingered).to.exist;

      // Second transfer with same nonce should fail (generic revert for nonce)
    ...
    it('should respect transaction limits for non-exempt users', async function () {
      ...
      await expect((fia.connect(alice)).protectedTransfer(bob.address, excessiveAmount, nonce))
        .to.be.reverted; // could be custom error or message depending on implementation
    });
    ...
    it('should increment nonce usage counter', async function () {
      ...
      // Use sequential nonces, advance time to avoid cooldown
      for (let i = 1; i <= 5; i++) {
        await (fia.connect(alice)).protectedTransfer(bob.address, amount, i);
        // advance time by txCooldown to avoid 'Cooldown not met'
        const limits: any = await fia.txLimits();
        await ethers.provider.send('evm_increaseTime', [Number(limits.txCooldown) + 1]);
  ...
  describe('BatchTransfer Function', function () {
    ...
    it('should respect individual transaction limits per transfer', async function () {
      ...
      await expect((fia.connect(alice)).batchTransfer(recipients, amounts))
        .to.be.reverted; // accept either Insufficient balance or Transaction amount exceeds limit
    });
  ...
  describe('TransferWithData Function', function () {
    it('should emit TransferWithData event', async function () {
      ...
      await expect((fia.connect(alice)).transferWithData(bob.address, amount, data))
        ...
        .withArgs(alice.address, bob.address, amount, ethers.keccak256(data));
    });
